...
export default function ApplyNow() {
  ...
  return (
    <section id="invitation" className="relative overflow-hidden bg-[#080b16] py-24 sm:py-32 border-t border-[#d4af37]/20">
      ...
      <div className="relative mx-auto max-w-4xl px-6">
        {/* Hogwarts Acceptance Letter Scroll Styling */}
        <motion.div
        ...
        >
          ...
          <div className="max-w-2xl text-left">
            ...
            >
              You have received an official invitation to represent your team in the AWS Student Builder Group Hogwarts Hackfest at MNNIT Allahabad. Step forward, accept the invitation, and prepare for sorting.
            </Reveal>
            ...
            <Reveal delay={0.16} className="mt-8">
              ...
              >
                <span>ACCEPT THE INVITATION & GET SORTED</span>
                <span className="text-lg">✉️</span>
              </motion.a>
            </Reveal>
          </div>

          {/* Invitation Details Badges */}
          <motion.div
            variants={staggerContainer(0.08, 0.2)}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, amount: 0.4 }}
            className="mt-12 grid grid-cols-1 gap-4 border-t border-[#b89228]/40 pt-8 sm:grid-cols-3"
          >
            {DETAILS.map((d) => (
              <motion.div
                key={d.label}
                variants={staggerItem}
                className="rounded-lg border border-[#b89228]/50 bg-[#f4e8c1]/70 px-4 py-3 text-center shadow-sm"
              >
                <p className="font-display text-sm font-bold text-[#24170f]">{d.value}</p>
                <p className="mt-0.5 text-xs text-[#8c6a15] font-sans uppercase tracking-wider">{d.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
